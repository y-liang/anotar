...
export default class CardZoom extends React.Component {
  ...
  getLiChildren = () => {
    const imgWidth = 330;
    const imgHeight = 165;
    const imgBoxWidth = 334;
    const imgBoxHeight = 168;
    return dataArray.map((item, i) => {
      ...
      const liStyle = isEnter ? { width: 1332, height: 333, zIndex: 1 } : null;
      const liAnimation = isOpen ?
        ({ boxShadow: '0 1px 8px rgba(255, 85, 0, .25)' }) :
        ({ boxShadow: '0 0px 0px rgba(255, 85, 0, 0)' });
      ...
      aAnimation = isOpen ?
        ({
          ease: 'easeInOutCubic',
          left: isRight ? (imgBoxWidth * 2) - 2 : 0,
          width: 666,
          height: 333,
          top: 0,
        }) : aAnimation;
